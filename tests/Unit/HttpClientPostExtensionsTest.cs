namespace System.Net.Http
{
    ...
    public class HttpClientPostExtensionsTest
    {
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(() => ((HttpClient)null).PostAsProtoBufAsync(_uri, new SimpleType()));
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(() => _client.PostAsProtoBufAsync(_uri, new SimpleType(), null, CancellationToken.None));
        ...
        {
            var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => _client.PostAsProtoBufAsync((string)null, new SimpleType()));
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(() => ((HttpClient)null).PostAsProtoBufAsync(new Uri(_uri), new SimpleType()));
            Assert.Equal("client", exception.ParamName);
        }
        ...
        {
            var exception = await Assert.ThrowsAsync<ArgumentNullException>(() => _client.PostAsProtoBufAsync(new Uri(_uri), new SimpleType(), null, CancellationToken.None));
            Assert.Equal("model", exception.ParamName);
        }
        ...
        {
            var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => _client.PostAsProtoBufAsync((Uri)null, new SimpleType()));
            Assert.Equal("An invalid request URI was provided. The request URI must either be an absolute URI or BaseAddress must be set.", exception.Message);
        }
